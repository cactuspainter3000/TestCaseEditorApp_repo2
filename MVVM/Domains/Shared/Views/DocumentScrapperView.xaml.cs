...
{
    /// <summary>
    /// Self-contained Document Scrapper view that can be embedded as a tab anywhere.
    /// Automatically gets its ViewModel through DI when parent view creates it.
    /// </summary>
    public partial class DocumentScrapperView : UserControl
    {
        public DocumentScrapperView()
        {
            InitializeComponent();
            
            // Auto-resolve ViewModel through DI container for self-contained operation
            DataContext = App.ServiceProvider?.GetService(typeof(DocumentScrapperViewModel));
        }
    }
}