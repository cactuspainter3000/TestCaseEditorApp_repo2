...
namespace TestCaseEditorApp.Prompts
{
    ...
    public sealed class RequirementAnalysisPromptBuilder
    {
        ...
        public string GetSystemPrompt()
        {
            ...
            sb.AppendLine("You are a systems engineering expert evaluating system-level requirement quality.");
            sb.AppendLine();
            sb.AppendLine("RULE: Supplemental paragraphs and tables provided with each requirement are part of that requirement.");
            sb.AppendLine("Do NOT flag terms as unclear if they are defined in the supplemental content.");
            sb.AppendLine();

            sb.AppendLine("EVALUATE across 6 dimensions:");
            sb.AppendLine("1. Clarity – unambiguous, single interpretation, no vague/subjective terms");
            sb.AppendLine("2. Testability – measurable, observable pass/fail criteria, concrete thresholds");
            sb.AppendLine("3. Completeness – specifies WHAT not HOW, all conditions/contexts stated");
            sb.AppendLine("4. Atomicity – single cohesive need, not a bundle of sub-requirements");
            sb.AppendLine("5. Actionability – clear system action or state, not aspirational intent");
            sb.AppendLine("6. Consistency – ID/name match, consistent units and terminology");
            sb.AppendLine();
            sb.AppendLine("ANTI-FABRICATION (critical):");
            sb.AppendLine("DO NOT invent technical terms, system names, protocols, tolerances, or attributes");
            sb.AppendLine("unless they appear verbatim in the requirement text or supplemental content.");
            sb.AppendLine("Use [specify X] placeholders for anything that is missing but needed.");
            sb.AppendLine("SuggestedEdit must be complete requirement language – not instructions like '[define this here]'.");
            sb.AppendLine();
            sb.AppendLine("RECOMMENDATIONS:");
            sb.AppendLine("Provide at most 2 recommendations. Consolidate related issues into one comprehensive rewrite.");
            sb.AppendLine("Every recommendation MUST include a SuggestedEdit with the full rewritten requirement text.");
            sb.AppendLine("If supplemental content defines a vague term in the requirement, incorporate that definition.");
            sb.AppendLine();
            // Output format
            ...
            sb.AppendLine();
            sb.AppendLine(@"{");
            sb.AppendLine(@"  ""QualityScore"": <integer from 0-100>,");
            ...
            sb.AppendLine(@"    }");
            sb.AppendLine(@"  ],");
            sb.AppendLine(@"  ""FreeformFeedback"": ""<optional: meaningful insights not covered above, or empty string>""");
            sb.AppendLine(@"}");
            sb.AppendLine();
            ...
            sb.AppendLine();
            sb.AppendLine("Score 0-30:  Poor quality. Multiple critical issues. Not testable or actionable. Needs major revision.");
            sb.AppendLine("Score 31-50: Fair quality. Some clarity or completeness issues. Testability is unclear or limited.");
            sb.AppendLine("Score 51-70: Good quality. Minor issues present. Generally testable but could be more precise.");
            sb.AppendLine("Score 71-90: Very good quality. Clear, testable, complete. Only minor improvements needed.");
            sb.AppendLine("Score 91-100: Excellent quality. Exemplary requirement. Clear, atomic, testable, complete, actionable.");
            sb.AppendLine();
            sb.AppendLine("SEVERITY: High=prevents verification, Medium=makes verification difficult, Low=minor improvement.");
            sb.AppendLine("Return ONLY the JSON object. No preamble, no markdown, no code fences.");
        ...
        public static bool IsValidJsonStructure(string json)
        {
            try
            {
                ...
                if (score.GetInt32() < 0 || score.GetInt32() > 100) return false;
