...
namespace TestCaseEditorApp.Prompts
{
    ...
    public sealed class RequirementAnalysisPromptBuilder
    {
        ...
        public string GetSystemPrompt()
        {
            ...
            sb.AppendLine("Evaluate the quality of system-level requirements using all provided information, including requirement text, supplemental paragraphs, and tables.");
            sb.AppendLine();

            sb.AppendLine("CRITICAL RULES:");
            sb.AppendLine("1. Treat provided tables and supplemental content as part of the requirement context.");
            sb.AppendLine("2. Do not mark a term ambiguous or unclear if it is defined in supplemental content.");
            sb.AppendLine("3. Do not invent, assume, or add technical details that are not present in the requirement text, tables, or supplemental material.");
            sb.AppendLine("4. If required details are missing, use bracketed placeholders like [specify time limit] instead of guessing.");
            sb.AppendLine("5. Use only 1-2 total recommendations maximum. Consolidate related issues into one comprehensive rewrite whenever possible.");
            sb.AppendLine("6. Every recommendation must include a SuggestedEdit containing complete requirement language, not instructions.");
            sb.AppendLine();

            sb.AppendLine("EVALUATE THESE DIMENSIONS:");
            sb.AppendLine("- Clarity & Precision: ambiguous, vague, subjective, or undefined terms");
            sb.AppendLine("- Testability & Verifiability: measurable criteria, observable outcomes, pass/fail basis");
            sb.AppendLine("- Completeness: missing conditions, modes, assumptions, dependencies, or constraints");
            sb.AppendLine("- Atomicity & Focus: multiple requirements combined or hidden sub-requirements");
            sb.AppendLine("- Actionability: unclear required behavior or non-implementable wording");
            sb.AppendLine("- Consistency & Traceability: ID/name/content mismatch, inconsistent terminology or units");
            sb.AppendLine();

            sb.AppendLine("REWRITE GUIDANCE:");
            sb.AppendLine("- Provide concrete rewrites using available source information.");
            sb.AppendLine("- If supplemental content defines vague terms, incorporate those details directly into the rewritten requirement.");
            sb.AppendLine("- If exact values are missing, use bracketed placeholders with concise guidance.");
            sb.AppendLine("- SuggestedEdit and ImprovedRequirement must be complete requirement statements suitable for implementation.");
            sb.AppendLine();

            sb.AppendLine("HALLUCINATION CHECK:");
            sb.AppendLine("- Use NO_FABRICATION if you used only source-provided terms and concepts.");
            sb.AppendLine("- Use HELPFUL_ELABORATION if you used bracketed placeholders or examples to show what must be specified.");
            sb.AppendLine("- Use FABRICATED_DETAILS only if technical details were added that were not present in the source.");
            sb.AppendLine();
            ...
            sb.AppendLine(@"{");
            sb.AppendLine(@"  ""OriginalQualityScore"": <integer 1-10>,");
            sb.AppendLine(@"  ""HallucinationCheck"": ""<NO_FABRICATION|HELPFUL_ELABORATION|FABRICATED_DETAILS>"",");
            ...
            sb.AppendLine(@"      ""Description"": ""<concise issue description>"",");
            sb.AppendLine(@"      ""Fix"": ""<past tense summary of what was improved>""");
            sb.AppendLine(@"    }");
            sb.AppendLine(@"  ],");
            ...
            sb.AppendLine(@"      ""SuggestedEdit"": ""<complete improved requirement text>""");
            sb.AppendLine(@"    }");
            sb.AppendLine(@"  ],");
            sb.AppendLine(@"  ""ImprovedRequirement"": ""<complete rewritten requirement>"",");
            sb.AppendLine(@"  ""FreeformFeedback"": ""<optional additional insight or empty string>""");
            sb.AppendLine(@"}");
