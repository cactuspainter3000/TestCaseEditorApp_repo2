...
try {
    ...
    $embeddingResponse = Invoke-RestMethod -Uri "http://127.0.0.1:11434/api/embeddings" -Method Post -Body $embeddingPayload -ContentType "application/json" -TimeoutSec 30
...
if ($freeRAM -gt 4) {
    ...
    Write-Host "   Low memory - may affect embedding performance" -ForegroundColor Yellow
}
